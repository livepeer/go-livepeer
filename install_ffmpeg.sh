...
if [ ! -e "$ROOT/x265" ]; then
  ...
  cd build/linux/
  cmake -DCMAKE_INSTALL_PREFIX=$ROOT/compiled -DENABLE_SHARED=false -DENABLE_CLI=OFF -DCMAKE_VERBOSE_MAKEFILE=ON -DCMAKE_OSX_DEPLOYMENT_TARGET=11.0 -G "Unix Makefiles" ../../source
  make
  make install
fi
# VP8/9 support
...
if [ ! -e "$ROOT/ffmpeg/libavcodec/libavcodec.a" ]; then
  ...
  ./configure ${TARGET_OS:-} --fatal-warnings \
    ...
    --enable-encoder=libx265,libvpx_vp8,libvpx_vp9 --enable-decoder=hevc,libvpx_vp8,libvpx_vp9 --enable-libx265 --enable-libvpx \
