...
type TranscoderClient interface {
	...
	RegisterAIWorker(ctx context.Context, in *RegisterRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[NotifyAIJob], error)
}
...
func (c *transcoderClient) RegisterAIWorker(ctx context.Context, in *RegisterRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[NotifyAIJob], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &Transcoder_ServiceDesc.Streams[1], Transcoder_RegisterAIWorker_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
	x := &grpc.GenericClientStream[RegisterRequest, NotifyAIJob]{ClientStream: stream}
	if err := x.ClientStream.SendMsg(in); err != nil {
		return nil, err
	}
	if err := x.ClientStream.CloseSend(); err != nil {
...
// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type Transcoder_RegisterAIWorkerClient = grpc.ServerStreamingClient[NotifyAIJob]
...
type TranscoderServer interface {
	...
	// Called by the transcoder to register a `RemoteAIWorker` worker to an orchestrator
	// notifies the registered `RemoteAIWorker` of AI jobs as they come in.
	RegisterAIWorker(*RegisterRequest, grpc.ServerStreamingServer[NotifyAIJob]) error
...
func (UnimplementedTranscoderServer) RegisterAIWorker(*RegisterRequest, grpc.ServerStreamingServer[NotifyAIJob]) error {
...
func _Transcoder_RegisterAIWorker_Handler(srv interface{}, stream grpc.ServerStream) error {
	...
	return srv.(TranscoderServer).RegisterAIWorker(m, &grpc.GenericServerStream[RegisterRequest, NotifyAIJob]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type Transcoder_RegisterAIWorkerServer = grpc.ServerStreamingServer[NotifyAIJob]
