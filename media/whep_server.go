...
func (s *WHEPServer) CreateWHEP(ctx context.Context, w http.ResponseWriter, r *http.Request, mediaReader io.Reader) {
	...
	for _, track := range tracks {
		...
		case *mpegts.CodecH264:
			...
			mpegtsReader.OnDataH264(track, func(pts, dts int64, au [][]byte) error {
				...
				// TODO timestamp truncation going from int64 to uint32
		...
		case *mpegts.CodecOpus:
			continue // skip for now due to sync latency
			webrtcTrack, err := NewLocalVideoTrack(
