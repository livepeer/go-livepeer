...
func mergeDiscoveryRunners(entry *remoteDiscoveryMergeEntry, discovery remoteDiscoveryEntry, eligibleCaps map[string]bool) {
	...
	for _, r := range discovery.Runners {
		...
		if !remoteDiscoveryRunnerEligible(r, eligibleCaps) {
			continue
		}
...
func remoteDiscoveryRunnerEligible(runner runner.LiveRunnerDiscoveryRunner, eligibleCaps map[string]bool) bool {
