...
func submitTextToImage(ctx context.Context, params aiRequestParams, sess *AISession, req worker.TextToImageJSONRequestBody) (*worker.ImageResponse, error) {
	...
	if monitor.Enabled {
<<<<<<< Updated upstream
		monitor.AiJobProcessed(ctx, "text-to-image", *req.modelId, monitor.AIJobInfo{LatencyScore: sess.LatencyScore, PricePerUnit: float64(sess.OrchestratorInfo.GetPriceInfo().PricePerUnit)})
=======
		monitor.AiJobProcessed(ctx, "text-to-image", *req.ModelId, monitor.AIJobInfo{LatencyScore: sess.LatencyScore, PricePerUnit: float64(sess.OrchestratorInfo.GetPriceInfo().PricePerUnit)})
>>>>>>> Stashed changes
	}
