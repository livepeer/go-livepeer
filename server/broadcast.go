...
type BroadcastConfig struct {
	maxPrice              *big.Rat
	maxPricePerCapability map[uint32]map[string]*big.Rat
	mu                    sync.RWMutex
}
...
func (cfg *BroadcastConfig) GetCapabilitiesMaxPrice(caps *net.Capabilities) *big.Rat {
	...
	for capabilityInt, constraints := range caps.Constraints {
		for modelID, _ := range constraints.Models {
			capPrice := cfg.GetCapabilityMaxPrice(capabilityInt, modelID)
...
func (cfg *BroadcastConfig) GetCapabilityMaxPrice(cap uint32, modelID string) *big.Rat {
	models, ok := cfg.maxPricePerCapability[uint32(cap)]
	if ok {
		_, modelOk := models[modelID]
		if modelOk {
			return cfg.maxPricePerCapability[uint32(cap)][modelID]
		} else {
			_, hasDefault := models["default"]
			if hasDefault {
				return cfg.maxPricePerCapability[uint32(cap)]["default"]
			}
		}
	}
