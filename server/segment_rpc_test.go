...
func TestSubmitSegment_Success(t *testing.T) {
	...
	noNameSeg.Duration = 2.0
	tdata, err = SubmitSegment(s, noNameSeg, 0)
	assert.Nil(err)
	latencyScore1 := tdata.LatencyScore

	noNameSeg.Duration = 4.5
	tdata, err = SubmitSegment(s, noNameSeg, 0)
	assert.Nil(err)
...
func TestUpdateOrchestratorInfo(t *testing.T) {
	assert := assert.New(t)

	sess := &BroadcastSession{PMSessionID: "foo"}
	...
	updateOrchestratorInfo(sess, info)
	...
	updateOrchestratorInfo(sess, info)
	// Check that a new PM session is not create because OrchestratorInfo.TicketParams = nil
	assert.Equal("foo", sess.PMSessionID)
