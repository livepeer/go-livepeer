...
func main() {
	...
	in := flag.String("in", "/home/gpu/tvideo/vsource/playlist.m3u8", "Input m3u8 manifest file")
	...
	repeat := flag.Int("repeat", 1000000, "# of times benchmark will be repeated")
	...
	nvidia := flag.String("nvidia", "nvidia 0", "Comma-separated list of Nvidia GPU device IDs (or \"all\" for all available devices)")
	netint := flag.String("netint", "", "Comma-separated list of NetInt device GUIDs (or \"all\" for all available devices)")
	outPrefix := flag.String("outPrefix", "/home/gpu/dtmp/", "Output segments' prefix (no segments are generated by default)")
		...
		for i := 0; i < *concurrentSessions; i++ {
			...
			go func(k int, wg *sync.WaitGroup) {
				for j, v := range pl.Segments {
					...
					_, srcinfo, _ := ffmpeg.GetCodecInfo(in.Fname)
					for _, o := range out {
						_, resinfo, _ := ffmpeg.GetCodecInfo(o.Oname)
