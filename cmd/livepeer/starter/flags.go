...
func NewLivepeerConfig(fs *flag.FlagSet) LivepeerConfig {
	...
	cfg.PricePerUnit = fs.String("pricePerUnit", "0", "The price per 'pricingUnitSize' amount of work units. Can be specified in wei or a custom currency in the format <price><currency> (e.g. 0.50USD). When using a custom currency, a corresponding price feed must be configured with -priceFeedAddr")
	// Shared by O's pricePerUnit and B's maxPricePerUnit.
	fs.StringVar(cfg.PixelsPerUnit, "pricingUnitSize", *cfg.PixelsPerUnit, "Number of work units in one pricing unit; pricePerUnit is the price for this many work units. Set > 1 for finer price granularity than 1 wei per work unit.")
	fs.StringVar(cfg.PixelsPerUnit, "pixelsPerUnit", *cfg.PixelsPerUnit, "[Deprecated] Use -pricingUnitSize. Amount of pixels per unit; set > 1 for smaller price granularity than 1 wei / pixel.")
