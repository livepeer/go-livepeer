...
func NewLivepeerConfig(fs *flag.FlagSet) LivepeerConfig {
	...
	cfg.HwMinVRAMGB = fs.Int("hwMinVRAM", *cfg.HwMinVRAMGB, "Minimum GPU VRAM in GB required for orchestrator selection (0 = no filter)")
	cfg.HwIncludeGPUs = fs.String("hwIncludeGPUs", *cfg.HwIncludeGPUs, "Comma-separated list of GPU name substrings to allow (e.g., \"H100,A100\"). If set, only orchestrators with matching GPUs are selected")
	cfg.HwExcludeGPUs = fs.String("hwExcludeGPUs", *cfg.HwExcludeGPUs, "Comma-separated list of GPU name substrings to exclude (e.g., \"K80,V100\")")
