...
func NewLivepeerConfig(fs *flag.FlagSet) LivepeerConfig {
	...
	cfg.MaxPricePerCapability = fs.String("maxPricePerCapability", *cfg.MaxPricePerCapability, `json list of prices per capability/model or path to json config file. Use "model_id": "default" to price all models in a pipeline the same. Example: {"capabilities_prices": [{"pipeline": "live-video-to-video", "model_id": "noop", "price_per_unit": 1000, "pixels_per_unit": 1}, {"pipeline": "live-video-to-video", "model_id": "default", "price_per_unit": 1200, "pixels_per_unit": 1}]}`)
	...
	cfg.AIWorker = fs.Bool("aiWorker", *cfg.AIWorker, "Set to true to run an AI worker alongside the orchestrator (requires -orchestrator)")
	...
	cfg.AIModels = fs.String("aiModels", *cfg.AIModels, "Set models (pipeline:model_id) for AI worker to load upon initialization; only the live-video-to-video pipeline is supported")
	...
	cfg.AIRunnerImageOverrides = fs.String("aiRunnerImageOverrides", *cfg.AIRunnerImageOverrides, `Specify overrides for the Docker images used by the AI runner. Example: '{"live": {"another-pipeline": "livepeer/ai-runner:another-pipeline-v1.0"}}'`)
