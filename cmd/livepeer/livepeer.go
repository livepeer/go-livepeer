...
func parseLivepeerConfig() starter.LivepeerConfig {
	...
	cfg.AIMinRunnerVersions = flag.String("aiMinRunnerVersions", *cfg.AIMinRunnerVersions, `JSON specifying the min runner versions for each pipeline. Example: '[{"model_id": "noop", "pipeline": "live-video-to-video", "minVersion": "0.0.2"}]'; if not set, the runner's min version is used"`)
