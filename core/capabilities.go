...
// ConstrainedModelID returns a deterministic constrained model ID across all
// per-capability constraints. When multiple capabilities have constrained
// models, it returns the lexicographically first model ID. Returns "" when
// no model is constrained.
func (bcast *Capabilities) ConstrainedModelID() string {
	if bcast == nil {
		return ""
	}
	modelIDs := []string{}
	for capability := range bcast.constraints.perCapability {
		if modelID := bcast.ModelIDForCapability(capability); modelID != "" {
			modelIDs = append(modelIDs, modelID)
		}
	}
	if len(modelIDs) == 0 {
