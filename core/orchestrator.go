...
func (n *LivepeerNode) transcodeSeg(ctx context.Context, config transcodeConfig, seg *stream.HLSSegment, md *SegTranscodingMetadata) *TranscodeResult {
	...
	if n.TranscoderManager != nil && n.TranscoderManager.hiveClient != nil {
		...
		// Create unique job ID by combining session ID and segment sequence number
		jobID := fmt.Sprintf("%s-%d", md.AuthToken.SessionId, seg.SeqNo)
		...
		if _, err := n.TranscoderManager.hiveClient.CreateJob(ctx, jobID, createJobReq); err != nil {
	...
	if n.TranscoderManager != nil && n.TranscoderManager.hiveClient != nil {
		...
		if _, err := n.TranscoderManager.hiveClient.CompleteJob(ctx, fmt.Sprintf("%s-%d", md.AuthToken.SessionId, seg.SeqNo), completeJobReq); err != nil {
