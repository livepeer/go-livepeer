...
ffmpeg -f lavfi -i color=white:s=48x48 -vframes 5  -c:v libx265 -f mpegts - | gzip -9 | xxd -i | awk 'NR > 1 { print prev } { prev=$0 } END { ORS=""; print }' >> $FILE
echo "}" >> $FILE

echo "var testSegment_VP8 = []byte{" >> $FILE
ffmpeg -f lavfi -i color=white:s=48x48 -vframes 5  -c:v libvpx -f webm - | gzip -9 | xxd -i | awk 'NR > 1 { print prev } { prev=$0 } END { ORS=""; print }' >> $FILE
echo "}" >> $FILE

echo "var testSegment_VP9 = []byte{" >> $FILE
ffmpeg -f lavfi -i color=white:s=48x48 -vframes 5  -c:v libvpx-vp9 -f webm - | gzip -9 | xxd -i | awk 'NR > 1 { print prev } { prev=$0 } END { ORS=""; print }' >> $FILE
echo "}" >> $FILE
